...
> 版本 v2.0 · 2026-09-10 · 适用范围：本项目全部后续开发与功能优化
...
- 真实地图瓦片 / 在线地图 SDK（当前为自绘 SVG + OSM 静态地理要素，见 ADR-007）。
...
- **v2.0 · 2026-09-10**：
  ...
  - ③ 地图建模维持 ADR-007「自绘 SVG + OSM 静态地理要素」方案不变（换真实瓦片需 key / 联网 / GCJ-02 偏移，与离线免 key 原则冲突）。

- **v1.9 · 2026-09-10**：
  - ① 后台反馈查看页 `/admin` 改为跟随应用三语切换（`admin.*` 文案入 `messages.json`，支持 zh-CN / ms-MY / en-US），并在顶栏加语言切换入口。
...
- **v1.7 · 2026-09-10**：
  ...
  - ② 默认语言改为马来语（语言存储键升级为 `srv-lang-v2`，新老访客首次进入均为马来语）；语言切换顺序调整为 马来语 / 中文 / 英语。
  - ③ 移除首页与导览页右上角「设置」齿轮图标，设置入口统一由底部导航进入。

- **v1.6 · 2026-09-09**：
  - ①「关于我们」页按《关于我们【中文版】.docx》重写为「项目背景 / 团队 / 项目目的」三段式富文本，三语文案补全（`messages.json` `about.*` 共 20 key）。
  - ② 导览页地图升级为「自绘 SVG + 导入 OSM 真实地理要素」——水系/公园绿地/城区块/主干道/兵马俑博物馆建筑烘焙为静态 `client/src/lib/mapFeatures.json`（© OSM ODbL），明城墙/地铁线手绘，三大景点改 3D 图钉标记（照片 + 专属图标 + 投影）；更新 ADR-007。
  - ③ `.gitignore` 新增忽略 `*.tsbuildinfo` 构建缓存。
