...
> 版本 v1.5 · 2026-09-06 · 适用范围：本项目全部后续开发与功能优化
...
导览页（手绘西安示意地图[城墙/渭河/地铁线/路网 + 真实照片标记 + 缩放拖拽] + 点击两个景点规划通行路线 + 内嵌播放条）；语言设置 / 设置总页 / 字体调节 / 问题反馈（文字+图片）/ 关于我们，含全部跳转链路。
...
| 地图 | 手绘西安示意 SVG（真实经纬度投影 + 城墙/渭河/地铁线/路网 + 真实照片标记 + 缩放/拖拽） | 免外部 API key、完全离线 |
...
| ADR-007 | 地图用**手绘示意 SVG**（真实经纬度投影 + 城墙/渭河/地铁线/路网 + 真实照片标记）而非高德/Leaflet | 免 key、完全离线、够用 | 需真实定位/导航时换真实瓦片 |
